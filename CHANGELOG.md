...
- **G5 cross-model envelope chain 검증**: base-ref `uw-gate verify-envelope --task N`이 candidate ancestry, charter/config/provider/model/fresh binding, frozen 로그, probe 로그, add-only chain과 final evidence blob을 검사한다. fake CLI 결정론 e2e는 양방향 mapping, config drift, same-provider, timeout, malformed output, probe 삭제, hash 재계산 chain 재배열을 fail-close로 고정한다. 비용 발생 actual provider semantic-mutant 검증은 CI 밖 `test/live-verifier-smoke.sh`로 분리했다. envelope는 작성자 attestation이 아니라 tamper-evidence라는 한계를 문서화했다.

...
- **저장소 self-CI** [품질 게이트]: `test/e2e-gates.sh`(권위 gate 17시나리오) + `test/e2e-lifecycle.sh`(lifecycle 11단언) + `.github/workflows/self-ci.yml`(PR/push마다 `sh -n` + `shellcheck --severity=error` + 두 e2e). 배포하는 강제 스크립트와 lifecycle 계약이 실제로 작동하는지 매 변경 검증한다(적용 대상의 merge 게이트 `uw-gate.yml`과는 별개).
...
### Fixed
- **P8 lifecycle·CI 배선 정합**: intake 산출물을 task-start 계약 baseline commit에 포함하고, Stage 독립 검증은 `git write-tree`/`commit-tree` candidate SHA의 detached worktree에서 수행한다. final은 `awaiting_merge`로 남겨 PR CI의 charter 결박을 유지하며 `done`은 인간 merge 사실에서만 도출한다. 권위 workflow가 base-ref `check-gates.sh`와 `uw-gate`를 모두 실행하고, G4는 escalation history↔label 제거 event·외부 actor·versioned clear artifact·post-escalation APPROVED review를 fail-close·1:1로 대조한다. G5는 baseline MISS·HEAD PASS·mutant MISS를 clean worktree에서 재실행하고 frozen verify drift를 차단한다. self-CI를 lifecycle 11개 + gate 17개 시나리오로 확장했다.

...
- **저장소 self-CI** [품질 게이트]: `test/e2e-gates.sh`(권위 gate 32시나리오) + `test/e2e-lifecycle.sh`(lifecycle 29단언) + `.github/workflows/self-ci.yml`(PR/push마다 `sh -n` + `shellcheck --severity=error` + workflow YAML parse + 두 e2e). 배포하는 강제 스크립트와 lifecycle 계약이 실제로 작동하는지 매 변경 검증한다(적용 대상의 merge 게이트 `uw-gate.yml`과는 별개).

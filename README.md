...
| [`task-stage-report`](src/templates/mydocs/skills/task-stage-report/SKILL.md) | 각 Stage 종료 시 | candidate commit의 격리 checkout에서 독립 검증. OK면 다음 Stage로 자동 진행 |
...
| 장치 | 기준 | 동작 |
|---|---|---|
| 독립 검증 게이트 | charter 수용·검증 기준 (모델 주관 아님) | Stage 구현 파일을 임시 candidate commit으로 snapshot하고, **구현자와 분리된 독립 검증**이 그 SHA의 깨끗한 checkout에서 동결 명령 재실행 + 적대 프로브로 OK/MISS 판정 |
...
merge gate는 base ref에서 gate와 helper를 모두 로드한다. G4는 append-only task escalation마다 GitHub `needs-human` labeled→unlabeled event·외부 `User`·현재 PR clear artifact·그 뒤 최신 HEAD approval을 1:1 대조한다. G5는 BASE 직후 product 변경 없는 계약 baseline MISS, HEAD PASS, 실행별 빈 HOME과 별도 clone의 mutant 주입 후 MISS, verify 동결, 구조화 evidence blob을 no-secret 환경에서 재검사한다. PR head는 `awaiting_merge`로 남으며 `done`은 인간 merge 이후 `MERGED`+non-null merge commit 사실에서만 도출한다.

...
그 사이의 모든 진행 판단은 charter 기준 자동 검증으로 대체됩니다. AI는 Stage를 구현하고 **반대 provider의 fresh 외부 모델**이 OK/MISS를 판정합니다. MISS면 같은 Stage에서 자기수정하며, charter급 사건이 생길 때만 사람을 부릅니다(에스컬레이션).

| Ultra-Waterfall의 핵심 | 의미 |
|---|---|
| 시작에서만 방향 결정 | 목표·범위·수용기준은 인테이크에서 charter로 확정·잠금합니다. |
| 자동 검증 = 승인 대체 | 단계 승인 게이트 대신, 구현자와 반대 provider가 charter 기준 OK/MISS를 fresh 판정합니다. |
...
- **중간의 교차 모델 검증**: Claude 구현이면 Codex, Codex 구현이면 Claude가 매 Stage 새 비영속 세션에서 판정하고, 검증 MISS를 OK로 넘기지 못하게 막습니다.
...
task-start (자동: 이슈·브랜치·오늘할일·구현계획)
   ...
   ▼
┌─ 자율 LOOP ──────────────────────────────────────┐
│  Stage 계획 → 구현 → 반대 모델 fresh 검증(OK/MISS) → _stage 기록 │
│     ├ OK   → 다음 Stage 자동 진행                    │
...
- **Stage**: LOOP의 한 회전(`계획 → 구현 → 교차 모델 검증 → 기록`). 검증과 보고를 한 번에 끝낼 수 있는 크기로 나눕니다.
- **자동 검증 게이트**: Stage 종료 시 charter 수용·검증 기준에 대한 객관 검증과 반대 provider의 적대적 OK/MISS 판정이 인간 승인을 대체합니다. 같은 provider fallback은 없습니다.
...
```text
https://github.com/Y-Park/ultra-waterfall 이 저장소의 ultra-waterfall 방법론을 내 저장소에 적용해줘.
```
...
먼저 https://github.com/Y-Park/ultra-waterfall 이 저장소의 ultra-waterfall 방법론을 이 저장소에 적용해줘.
...
| [`task-start`](src/templates/mydocs/skills/task-start/SKILL.md) | charter 잠금 후 (자동) | 브랜치·오늘할일·구현계획서 + 구현 전 계약 baseline commit |
| [`task-stage-report`](src/templates/mydocs/skills/task-stage-report/SKILL.md) | 각 Stage 종료 시 | immutable candidate를 반대 provider의 disposable bundle에서 fresh 검증. OK면 다음 Stage로 자동 진행 |
| [`task-final-report`](src/templates/mydocs/skills/task-final-report/SKILL.md) | 전 수용기준 OK (종료) 시 | `awaiting_merge` 기록 + Open PR 게시 (인간 검토 대상) |
...
| 장치 | 기준 | 동작 |
|---|---|---|
| 교차 모델 fresh 게이트 | charter 수용·검증 기준 | Stage 구현 파일을 reachable candidate commit으로 snapshot하고, **반대 provider**가 disposable bundle의 `uw-probe`로 동결 명령 재실행 + 적대 프로브를 수행해 OK/MISS 판정 |
| 검증 변별력(teeth) | 각 must-fix AC | 기계검증 *가능* ≠ 충분. 막으려는 위반(mutant) 주입 시 MISS임을 인테이크에서 증명(**red-first + teeth**)해야 charter 잠금. 검증이 mutant를 통과하면 잡을 때까지 보강 |
...
| loop-state 0.4.0 | `.ultra-waterfall/task-{N}.json` (task별) | LOOP 상태·가드와 구현/검증 provider, task-frozen model·effort·config hash, envelope chain head 기록 |

merge gate는 base ref에서 gate와 helper를 모두 로드한다. G4는 append-only task escalation마다 GitHub `needs-human` labeled→unlabeled event·외부 `User`·현재 PR clear artifact·그 뒤 최신 HEAD approval을 1:1 대조한다. G5는 BASE 직후 product 변경 없는 계약 baseline MISS, HEAD PASS, 실행별 빈 HOME과 별도 clone의 mutant MISS에 더해 task-frozen config, provider 반대성, reachable candidate·charter, fresh 호출, 독립 probe, add-only envelope chain을 no-secret 환경에서 재검사한다. PR head는 `awaiting_merge`로 남으며 `done`은 인간 merge 이후 `MERGED`+non-null merge commit 사실에서만 도출한다.

검증 model과 추론 수준은 [`.ultra-waterfall/verifier/config.json`](src/templates/.ultra-waterfall/verifier/config.json)에서 바꿉니다. 이 파일은 적용 시 `preserve`되며 task-start에서 해시가 동결됩니다. 변경은 다음 task부터 적용됩니다. 교차 모델은 상관된 맹점을 줄이는 위험 분산 장치이지 결함 부재의 증명은 아니며, envelope도 외부 호출 주체의 암호학적 attestation이 아니라 tamper-evidence입니다.
...
│   ├── bin/uw-verifier             반대 provider fresh 검증 harness
│   ├── verifier/                   사용자 config(preserve) + schema/prompt
...
| 진행 판단 | 사람의 단계별 승인 | charter 기준 **반대 provider fresh 검증** |
...
| 단계적 진행 | Stage 단위 구현·교차 모델 검증·기록 | 복잡한 작업을 검증 가능한 단위로 분할 |
...
ultra-waterfall은 **MIT** 라이선스입니다([LICENSE](LICENSE), Copyright © 2026 Y-Park).

본 방법론은 MIT 라이선스인 [hyper-waterfall](https://github.com/postmelee/hyper-waterfall)에서 **파생**되었습니다. MIT 요건에 따라 hyper-waterfall의 원 저작권·허가 고지를 [THIRD_PARTY_LICENSE](THIRD_PARTY_LICENSE)에 원문 그대로 보존합니다(계보 상세는 [계보 섹션](#계보-hyper-waterfall에서-ultra-waterfall로)).

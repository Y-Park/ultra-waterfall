...
| 장치 | 기준 | 동작 |
|---|---|---|
| 독립 검증 게이트 | charter 수용·검증 기준 (모델 주관 아님) | Stage 종료 시 **구현자와 분리된 독립 검증**이 "충족 못 하는 반례를 찾아라"(refute-first) 태도로, 깨끗한 체크아웃에서 동결 명령 재실행 + **자기 적대 프로브**(실패공간 추가 공격)로 OK/MISS 판정. MISS를 OK로 보고 금지 |

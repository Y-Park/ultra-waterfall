...
- **charter(방향 명세)**: 인테이크의 산출물이자 자율 LOOP가 따르는 불변 계약. 목표/비목표/범위/제약/가정/리스크/수용기준/검증기준/자기수정 한도 N/에스컬레이션 조건/전역 가드를 담습니다. 위치는 `mydocs/plans/task_{milestone}_{issue}_charter.md`이고, 같은 내용을 GitHub Issue 본문에도 기록합니다. 형식은 [`charter.md`](templates/mydocs/_templates/charter.md) 템플릿을 따릅니다.
...
LOOP 규범 전체는 [`ultra_loop_guide.md`](templates/mydocs/manual/ultra_loop_guide.md), 인테이크 절차는 [`task-intake`](templates/mydocs/skills/task-intake/SKILL.md) SKILL을 따릅니다.
...
AI는 [`docs/agent-entrypoint.md`](docs/agent-entrypoint.md)를 진입점으로 읽고, [`templates/manifest.json`](templates/manifest.json)이 정의한 **strict 범위 안에서 자율로** 파일을 복사·치환합니다(`strictManifest`). manifest가 정의하지 않은 파일은 만들지 않고, 기존 파일이나 사용자 수정과 충돌하는 후보만 사람에게 보고합니다.
...
두 경로 모두 적용 절차는 [`docs/lifecycle/adoption.md`](docs/lifecycle/adoption.md)를 따릅니다.
...
| SKILL | 사용하는 시점 | 역할 |
|---|---|---|
| [`task-intake`](templates/mydocs/skills/task-intake/SKILL.md) | 세션 시작 (인간 접점 1) | 추상 프롬프트를 charter로 구체화·확정·잠금. 자율 LOOP의 유일한 시작 게이트 |
| [`task-register`](templates/mydocs/skills/task-register/SKILL.md) | charter 확정 후 이슈가 없을 때 | charter를 본문으로 GitHub Issue 자동 등록 |
| [`task-start`](templates/mydocs/skills/task-start/SKILL.md) | charter 잠금 후 (자동) | 브랜치·오늘할일·charter 기반 구현계획서 생성 |
| [`task-stage-report`](templates/mydocs/skills/task-stage-report/SKILL.md) | 각 Stage 종료 시 | 자기검증 OK/MISS 판정·기록. OK면 다음 Stage로 자동 진행 |
| [`task-final-report`](templates/mydocs/skills/task-final-report/SKILL.md) | 전 수용기준 OK (종료) 시 | 최종 보고서 작성 + Open PR 게시 (인간 검토 대상) |
| [`pr-merge-cleanup`](templates/mydocs/skills/pr-merge-cleanup/SKILL.md) | 인간 merge 직후 (자동) | 이슈 close, 원격/로컬 브랜치·worktree 정리 |
| [`external-pr-review`](templates/mydocs/skills/external-pr-review/SKILL.md) | 외부 기여자 PR 검토 시 | `mydocs/pr/` 검토 문서, 검증, 권고(merge/수정/닫기) |
| [`todo`](templates/mydocs/skills/todo/SKILL.md) | 오늘할일 보드 갱신 시 | `mydocs/orders/yyyymmdd.md` 표 형식 갱신 |
...
| 장치 | 기준 | 동작 |
|---|---|---|
| 자동 검증 게이트 | charter 수용·검증 기준 (모델 주관 아님) | Stage 종료 시 OK/MISS 자기판정 + 객관 검증. MISS를 OK로 보고 금지 |
| 자기수정 한도 N | charter 기본 N=3 | MISS 시 같은 Stage에서 `진단→수정→재검증` 최대 N회 |
| 에스컬레이션 | N회 실패 / charter급 사건 | LOOP 멈춤 + 인간 호출 (loop-state, 실패 검증, 막힌 지점 동봉) |
| 전역 가드 | 총 Stage 상한(기본 8), 총 자기수정 상한(기본 12) | 누적치가 상한에 닿으면 자동 진행 중단·보고 |
| loop-state | `.ultra-waterfall/loop-state.json` | LOOP 현재 상태 기록 파일이자 향후 런타임 LOOP 러너 확장 훅 |

상세는 [`ultra_loop_guide.md`](templates/mydocs/manual/ultra_loop_guide.md), 자율 실행과 charter 경계 규율은 [`agent_code_hyperfall_rule_conflict.md`](templates/mydocs/manual/agent_code_hyperfall_rule_conflict.md)를 따릅니다.
...
폴더별 역할과 문서 파일명 규칙은 [`document_structure_guide.md`](templates/mydocs/manual/document_structure_guide.md)를 따르고, 산출물 출력 형식은 [`mydocs/_templates/`](templates/mydocs/_templates/)에 고정되어 있습니다. `mydocs/`는 작업 기억·운영 매뉴얼·조사 근거 보관용이며 대상 프로젝트의 공식 제품 문서 루트가 아닙니다. 제품/사용자/기여자/API/아키텍처/로드맵 문서는 charter의 문서 위치 판단을 거쳐 별도 task에서 다룹니다.

`_templates/`는 실제 산출물이 아니라 출력 형식의 진실 원천입니다. 각 SKILL은 산출물을 만들 때 먼저 해당 템플릿을 참조합니다. GitHub Issue 본문은 [`task.yml`](templates/.github/ISSUE_TEMPLATE/task.yml), PR 본문은 [`pull_request_template.md`](templates/.github/pull_request_template.md)를 기준으로 구조화합니다.
...
│   └── loop-state.json             자율 LOOP 상태 (인테이크 시 생성)
├── .github/
...
적용 범위와 충돌 처리는 [`templates/manifest.json`](templates/manifest.json)의 strict 모드를 따르며, 진입 절차는 [`docs/agent-entrypoint.md`](docs/agent-entrypoint.md)에 정리되어 있습니다.

...
merge gate는 base ref에서 gate와 helper를 모두 로드한다. G4는 task escalation마다 GitHub label 제거 event·외부 actor·versioned clear artifact·승인 review를 1:1 대조하고, G5는 계약 baseline MISS·HEAD PASS·mutant MISS와 verify script 동결을 별도 checkout에서 재실행한다. PR head는 `awaiting_merge`로 남으며 `done`은 인간 merge 이후 GitHub 사실에서만 도출한다.

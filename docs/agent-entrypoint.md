...
ultra-waterfall은 인간이 시작에서 방향만 잡아주고(인테이크) AI가 자율 LOOP를 도는 방법론이다. 방법론 설치(신규 적용)는 `templates/manifest.json`의 strict 범위 안에서 자율로 수행하고, 그 범위 밖 충돌만 인간에게 보고(에스컬레이션)한다.
...
1. `templates/manifest.json`을 읽고 적용 후보(copy/preserve/symlink)와 기존 target 충돌을 분류한다.

...
- 각 Stage 검증 명령은 담당 AC의 charter 검증 명령과 **동일**하다(위 커버리지 표). 단계 보고서 작성 전에 실행하고, 판정은 task-start에서 동결한 **반대 provider의 fresh 검증**으로 한다(`ultra_loop_guide.md` "자동 검증 게이트").
...
- 구현 provider, 반대 verifier provider, 요청 model/effort, config hash는 task 동안 고정한다. 같은 provider fallback이나 외부 session resume은 계획에 넣지 않는다.

...
- **단계별 완료보고서**: 한 Stage가 끝났을 때 `mydocs/working/`에 남기는 `_stage{N}.md` 보고서(교차 모델 판정·envelope 포함).
- **최종 결과보고서**: 모든 Stage가 끝난 뒤 `mydocs/report/`에 남기는 `_report.md` 보고서.
- **자동 검증 게이트**: Stage 종료 시 task-frozen 반대 provider의 fresh OK/MISS 판정. 인간 승인을 대체한다.
...
4. 각 Stage: 구현 → charter 기준 반대 provider fresh OK/MISS 판정 → `_stage{N}.md`·envelope/probe 작성 → reachable candidate와 evidence commit 연결.

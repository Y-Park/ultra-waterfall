...
- **독립 검증 주체**: 구현 대화이력을 공유하지 않는 별도 검증자에게 **charter(수용·검증 기준) + 변경 diff + 검증 명령 출력 로그**만 주고 OK/MISS를 재판정시킨다.
  - Claude Code/Codex: 서브에이전트(Agent/Task)를 새 컨텍스트로 띄워 검증 전용으로 호출한다.
  - 서브에이전트를 쓸 수 없으면 최소한 **구현과 검증을 같은 응답에서 하지 않고**, "이 Stage가 charter 기준을 충족하지 *못하는* 이유를 찾아라"는 적대적 프롬프트로 fresh-eyes 재판정한다.
...
검증이 변별력이 없을 수 있다(red-first 미확인). charter 검증이 미작업 상태에서 실제 MISS를 내는지(반례) 확인하라. 독립 검증자가 명령 약화 여부를 점검한다.

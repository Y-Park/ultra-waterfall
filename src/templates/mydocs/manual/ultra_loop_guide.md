...
2. `exit.code`로 분기:
   ...
   - `awaiting_merge` → PR이 open+ready면 인간 검토 대기. PR이 없거나 draft면 [`task-final-report`](../skills/task-final-report/SKILL.md) 8번의 publish push/create-or-edit/ready를 재실행해 복구한다(구현 LOOP로 돌아가지 않는다). closed-unmerged면 이유를 보존하고 에스컬레이션한다. `gh pr view --json state,mergeCommit` 출력을 `uw-gate merge-fact`가 통과하면 effective `done`으로 보고 cleanup한다.
   ...
   - legacy 0.3.0이 `implementing`/`verifying`/`correcting`이면 0.4.0으로 중간 변환하지 않는다. 기존 writer로 종료하거나 인간에게 에스컬레이션한 뒤 새 task부터 교차 모델 계약을 적용한다.
...
task-start (자동: 브랜치·오늘할일·구현계획서·loop-state 초기화·draft PR)
   ...
   ▼
┌─ LOOP ─────────────────────────────────────────────────────┐
│  [가드 검사] 구현 → 반대 모델 fresh 검증(OK/MISS) → 드리프트 → 기록·커밋 │
...
3. **Stage 반복**: [`task-stage-report`](../skills/task-stage-report/SKILL.md)를 Stage 순서대로 호출. 각 Stage는 가드검사→구현→candidate commit snapshot→반대 provider fresh 검증→드리프트점검→envelope·loop-state 기록·커밋.
...
## 자동 검증 게이트 (교차 모델 fresh 검증)

자동 검증이 인간 승인을 대체하므로, **구현한 주체와 같은 provider가 자기 결과를 채점하면 안 된다**(상관된 맹점과 확증편향 완화). Stage 종료와 통합검증은 task 시작 시 동결한 반대 provider로만 수행한다.

- **provider 반대성(필수)**: 구현자 Claude → 검증자 Codex, 구현자 Codex → 검증자 Claude다. 같은 provider fallback은 없고 반대 CLI 미설치·미인증·timeout·구조화 출력 실패는 fail-close한다. 교차 모델은 결함 감소를 보장하는 증명이 아니라 상관 위험을 분산하는 통제다.
...
- **불일치 처리**: 구현자의 기대와 교차 모델 검증 결과가 다르면 **OK가 아니라 MISS로 강등**하고 자기수정/에스컬레이션한다.
...
- `task-final-report`가 별도 **fresh 교차 모델 통합 검증**(charter 전 수용기준 일괄)을 한다. Stage 세션을 resume하지 않는다.
...
```json
{
  ...
  "verifier": {
    ...
    "implementerProvider": "codex | claude",
    "provider": "claude | codex",
    "configPath": ".ultra-waterfall/verifier/config.json",
    "configHash": "sha256:...",
    "model": "task-frozen requested model",
    "effort": "task-frozen requested effort",
    "chainHead": { "path": "mydocs/working/...verifier.json", "sha256": "..." }
  },
  "state": "planning | implementing | verifying | correcting | escalated | awaiting_merge",
  "lastVerification": { "phase": "stage | final", "stage": 0, "result": "OK | MISS", "by": "cross-model", "candidate": "commit SHA", "provider": "codex | claude", "model": "requested model", "evidence": "mydocs/working/...verifier.json#git:<blob id>" },
...
- legacy 0.3.0의 `state: done`/`exit.code: completed`는 historical completion을 읽기 위한 호환값이다. 진행 중 0.3.0 task는 중간 업그레이드하지 않으며 새 writer는 0.4.0과 `awaiting_merge`까지만 기록한다.
- `totalStages`/`selfCorrectionTotal`=가드용 누적값. `plannedStages`=계획값(예측용, 누적값과 구분).
- `history[]`=Stage별 append-only 궤적(사후 감사·재구성용).
- `verifier`는 task-start에서 동결한 반대 provider/effective model·effort/config와 envelope chain head다. `chainHead`는 null로 시작하고 의미론적 MISS를 포함한 매 외부 판정 뒤 갱신한다.
- `lastVerification.by`는 0.4.0에서 `cross-model`만 허용한다. `evidence`는 versioned verifier envelope의 `경로#git:<git hash-object 결과>`이며 CI가 HEAD blob과 chain head를 대조한다.
...
인간 개입은 **인테이크 1회 + 최종 PR 검토 1회**. 중간 Stage 승인·"승인 간주"는 없다. 그 사이 진행 판단은 charter 기준 반대 provider fresh 검증으로 대체하고, charter로 판단 불가한 사건만 에스컬레이션으로 인간을 부른다.
...
검증이 변별력(teeth)이 없을 수 있다. 두 가지를 확인하라: (1) **red-first** — 미작업 상태에서 실제 MISS를 내는가, (2) **teeth** — AC가 막으려는 *타당한 위반(mutant)*을 한 줄 주입했을 때 MISS를 내는가. mutant를 통과시키면 검증이 너무 약한 것(예: 경계 한 케이스만 보는 fixture) — 잡을 때까지 보강한다. 검증 약화 여부는 교차 모델 검증자가 점검한다. (teeth는 인테이크에서 잠그는 게 정석이고, 여기서 새로 발견되면 charter급 사건.)

...
6. **charter 확정·잠금**: 인간 확정 시 상태 `LOCKED` + 잠금 일시 기록. charter 본문 해시(`git hash-object`)를 **baseline**으로 **loop-state(`charterHash`)에만** 기록한다(charter 본문엔 해시를 넣지 않아 자기참조 회피 — CI가 `git hash-object`로 그대로 재검증 가능).
   - **CI 실행형 검증 emit(G5 강제)**: 잠금 시점에 각 must-fix AC의 frozen 검증을 task별 namespace에 생성한다. 이슈가 이미 있으면 `.ultra-waterfall/verify/task-{issue}/<ac>.sh`, 아직 없으면 `.ultra-waterfall/verify/pending-{slug}/<ac>.sh`를 사용한다. teeth mutant는 같은 디렉터리의 `<ac>.mutant.sh`이며, **위반을 worktree에 주입하고 exit 0**이어야 한다. CI가 그 뒤 frozen `<ac>.sh`의 MISS를 판정한다. charter 검증표 ↔ 해당 task namespace의 스크립트가 1:1 일치해야 한다. (계약 확정의 일부라 인테이크에서 수행 — LOOP 중 현재 task namespace는 동결된다.)
   - **CI 강제 AC 선언(G5 parity)**: charter의 `<!-- uw:verify-acs:begin -->`/`end` 블록에 위에서 emit한 검증 토큰(`<ac>`, 파일명과 동일)을 빠짐없이 적는다. CI가 이 선언 집합 ↔ 현재 task verify namespace의 `*.sh` 집합을 1:1 검사하므로(gap/orphan FAIL), 표·스크립트·이 블록 셋이 동일 AC 집합이어야 한다.
7. **인계**:
   ...
   - 이슈가 **이미 있으면** → charter를 `task_{milestone}_{issue}_charter.md`로 두고, `.ultra-waterfall/task-{issue}.json`을 `schemaVersion: 0.4.0`으로 생성(`charterHash`, `issue`, `milestone`, `charter`, `guards`, `verifier: null`, `state: planning`, `exit.code: running`, `updatedAt`)한 뒤 [`task-start`](../task-start/SKILL.md)로 인계. verifier snapshot은 구현 provider가 확정되는 task-start에서만 채운다.
   - charter·현재 task의 `.ultra-waterfall/verify/task-{issue}/`·loop-state는 이 시점에 아직 미커밋인 **expected intake artifacts**다. 다른 변경과 섞지 않으며, `task-start`가 구현 전 계약 baseline 커밋에 한 번에 포함한다.

## 산출물

- charter (`LOCKED`, 해시 baseline 기록)
- `.ultra-waterfall/verify/task-{issue}/<ac>.sh` + `<ac>.mutant.sh` 또는 채번 전 `pending-{slug}/` namespace (구현 전 red-first/teeth 계약)
...
- charter `uw:verify-acs` 선언 = 현재 task verify namespace의 `*.sh` 집합 (G5 parity: gap/orphan 없음)

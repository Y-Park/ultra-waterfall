...
5. **red-first 확인**: 각 AC 검증 명령이 **미작업 상태에서 실제로 MISS/실패**함을 확인한다(항진적·무의미 검증이 게이트를 무력화하는 것 방지). 출력 요약을 charter 검증표에 적는다.
6. **charter 확정·잠금**: 인간 확정 시 상태 `LOCKED` + 잠금 일시 기록. charter 본문 해시(`git hash-object` 또는 sha256)를 **baseline**으로 charter 머리말과 loop-state(`charterHash`)에 기록한다.
7. **인계**:

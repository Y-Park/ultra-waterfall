...
10. charter 파일명 확정 + loop-state 생성 + 인계
    ...
    - `.ultra-waterfall/task-{N}.json`을 `schemaVersion: 0.4.0`으로 생성한다(스키마는 `ultra_loop_guide.md`): `issue`=N, `milestone`, `charter`=새 경로, `charterHash`=charter baseline, `guards`(charter 값), `verifier: null`, `state: planning`, `exit.code: running`, `updatedAt`. 누적 카운터(`totalStages`/`selfCorrectionTotal`/`currentStageCorrections`)는 0. verifier snapshot은 구현 provider가 확정되는 task-start에서 채운다.
...
- `.ultra-waterfall/task-{N}.json`이 `schemaVersion: 0.4.0`으로 생성되고 `issue`/`charter`/`charterHash`/`guards`/`verifier: null`/`state: planning`/`exit.code: running`이 채워졌다.

...
- **배포 manifest**: ultra-waterfall release에 포함되는 파일, 적용 대상 경로, 업데이트 정책, checksum 상태를 정의하는 `templates/manifest.json`.
...
ultra-waterfall의 적용 기준은 `templates/manifest.json`이다. manifest는 적용 대상 파일, 적용 경로(source→target), 업데이트 정책을 정의한다.
...
- 적용 대상 프레임워크 파일과 적용 경로(source→target)를 나열한다. 예: `templates/AGENTS1.md -> AGENTS.md`.
...
- `templates/manifest.json`이 바뀌면 README의 적용 설명과 `docs/lifecycle/adoption.md`를 함께 확인한다.
...
- README나 `docs/agent-entrypoint.md`의 적용 설명이 바뀌면 manifest와 `docs/lifecycle/adoption.md` 설명을 함께 확인한다.

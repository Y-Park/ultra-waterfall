...
ultra-waterfall 방법론 자체를 새 저장소에 설치하는 작업은 [`docs/agent-entrypoint.md`](../../../docs/agent-entrypoint.md)와 [`docs/lifecycle/adoption.md`](../../../docs/lifecycle/adoption.md), `templates/manifest.json`을 기준으로 manifest strict 범위 안에서 수행한다. 설치가 끝나면 실제 작업은 아래 일반 타스크 절차(인테이크→LOOP)로 진행한다.

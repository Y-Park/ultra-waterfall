...
> 본 SKILL은 자율 task LOOP가 아니라 외부 기여 PR을 검토하는 별개 활동이다(인간 주도 동기 작업).
> 검토는 인간이 시작하고, 외부 기여의 수용(merge/close)은 인간이 결정한다.
> **자율 LOOP 진행 중(`exit.code: running`)에는 호출하지 않는다.** LOOP 도중 외부 PR이 유입되면 현재 task를 방해하지 말고 별도 세션/시점에 처리한다(LOOP의 자동 검증·가드와 외부 PR의 인간 승인 게이트를 섞지 않는다).
...
- 내부 타스크 PR(`publish/task{N}`)에는 본 SKILL 사용 금지 — 내부 타스크는 일반 LOOP 절차로 검토
- `gh` CLI 인증
...
8. 처리 완료 시 문서 보관 이동
   ...
   ```
9. 단일 또는 묶음 커밋 (외부 PR 검토는 내부 LOOP 단계 형식 강제 아님)
   ```bash
...
- 내부 LOOP 단계 절차(`_stage{N}.md`, `_report.md`) 형식을 외부 PR 문서에 강제 적용

...
description: |
  ...
  PR이 실제로 merge된 직후에만 호출. (울트라-워터폴)
---
...
- 대상 PR이 GitHub에서 실제 merged 상태로 확인되면 LOOP 종료 후처리로 자동 적용
- 본 SKILL을 직접 호출한 경우

## 사전 조건

- 대상 PR이 GitHub에서 실제 merged 상태 (`gh pr view {번호} --json state,mergeCommit`)
- 최종 PR merge는 인간이 결정한 단계이며, 본 절차는 그 merge가 확인된 이후에만 진입한다
...
- 다른 task 브랜치(`local/task{다른번호}`)나 메인 worktree 삭제

...
이 시점이 LOOP에서 유일하게 남는 인간 접점(인간 게이트 2)이다. 최종 보고서 작성과 PR 게시까지는 자율로 진행하고, PR 검토·merge만 작업지시자가 결정한다.
...
- 구현 계획서의 모든 Stage 종료, 각 단계 보고서 커밋 완료
- charter가 잠금 상태이며, 통합 검증(charter 전체 수용 기준) 통과 확인
- `local/task{N}`에 commit 안 된 변경 없음 또는 본 절차에서 함께 커밋할 것만 남아 있음

## 절차

1. 통합 검증: charter의 "수용 기준" 전체를 일괄 실행한다. 구현 계획서의 마지막 단계 "검증" 섹션 명령도 함께 확인한다.
   - 각 수용 기준은 OK/MISS로 판정한다. 하나라도 MISS면 PR 게시로 진행하지 않는다.
2. 최종 보고서 작성: `mydocs/report/task_m{milestone}_{N}_report.md`
   ...
   - 템플릿을 읽을 수 없는 경우에만 다음 최소 섹션을 fallback으로 사용한다:
     - 작업 요약 (이슈 링크, charter 링크, 마일스톤, 단계 수)
     - 변경 파일 목록과 영향 범위
     - 변경 전·후 정량 비교 (라인 수, 토큰, 검증 통과 등 적용 시)
     - 검증 결과 (charter 수용 기준별 OK/MISS)
     - 잔여 위험과 후속 작업
3. 오늘할일 갱신: `mydocs/orders/{yyyymmdd}.md`의 #{N} 행
   - 출력 형식은 `mydocs/_templates/orders.md`를 기준으로 한다.
   - 상태 `완료`로 변경, 비고에 `완료: HH:mm` 기록
4. LOOP 상태 갱신: `.ultra-waterfall/loop-state.json`의 `exit.code`를 `completed`로 설정한다(종료 시각과 통과한 수용 기준 요약 포함).
5. 변경 점검
   ```bash
   git status --short
   git diff --check
...
6. 최종 커밋 (Stage 마지막 + 최종 보고서를 묶을 수도, 보고서만 단일 커밋도 가능)
   ```bash
   git add mydocs/report/task_m{milestone}_{N}_report.md mydocs/orders/{yyyymmdd}.md .ultra-waterfall/loop-state.json
...
8. {BASE_BRANCH} 대상 Open PR 생성
   ...
   - 작업 문서(charter, 구현 계획서, 보고서)는 `HEAD_SHA` 기준 `https://github.com/{REPO_SLUG}/blob/{HEAD_SHA}/mydocs/...` URL로 연결
...
9. 작업지시자에게 PR URL 전달과 리뷰·merge 요청 (이 한 군데는 인간 게이트로 유지한다. merge 여부는 작업지시자가 결정한다.)

## 검증

- 모든 단계 보고서 + 최종 보고서 존재
- 최종 보고서가 `mydocs/_templates/final_report.md`의 필수 섹션을 채움
- charter 전 수용 기준이 OK로 판정됨 (MISS 0건)
- `.ultra-waterfall/loop-state.json`의 `exit.code`가 `completed`
...
- 통합 검증 실패(charter 수용 기준 MISS) 상태에서 PR 생성
